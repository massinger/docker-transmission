...
# image name
DOCKER_IMAGE="${DOCKER_USERNAME}/${DOCKER_IMAGE:-transmission}"
# "production" branch
...
docker build --build-arg VCS_REF="${VCS_REF}" \
             ...
             --tag "${DOCKER_IMAGE}:${docker_tag_prefix}${TRANSMISSION_VERSION}" \
             --file Dockerfile \
             .

## Image taaging
echo "${DOCKER_IMAGE}:${docker_tag_prefix}${TRANSMISSION_VERSION}" > ${build_tags_file}

# Tag images
for tag in $DOCKER_IMAGE_TAGS; do
  if [ -n "$tag" ]; then
    docker tag "${DOCKER_IMAGE}:${docker_tag_prefix}${TRANSMISSION_VERSION}" "${DOCKER_IMAGE}:${docker_tag_prefix}${tag}"
